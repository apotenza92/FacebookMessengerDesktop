...
```bash
# Test each VM
sshpass -p 'YOUR_VM_PASSWORD' ssh testuser@windows-vm.local "echo 'Windows VM connected'"
sshpass -p 'YOUR_VM_PASSWORD' ssh testuser@ubuntu-vm.local "echo 'Ubuntu VM connected'"
sshpass -p 'YOUR_VM_PASSWORD' ssh testuser@fedora-vm.local "echo 'Fedora VM connected'"
```
